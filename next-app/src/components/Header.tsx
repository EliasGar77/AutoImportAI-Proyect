...
export default function Header() {
  return (
    <header className="fixed top-4 left-0 w-full z-50 flex justify-center px-4 transition-all">
      <div className="w-full max-w-[1200px] bg-white/80 backdrop-blur-xl border border-white/40 shadow-[0_8px_30px_rgba(0,0,0,0.08)] rounded-full h-[70px] px-8 flex justify-between items-center transition-all">
        {/* Logo Left */}
        <div className="flex-1 flex justify-start">
          <Link href="/" className="flex items-center gap-3 group">
            <img src="/AutoImportAI-Proyect/assets/logo.png" alt="AutoImport Icon" className="h-[36px] w-auto object-contain mix-blend-multiply contrast-150 brightness-110 grayscale opacity-90 group-hover:scale-105 transition-transform" />
            <div className="flex flex-col justify-center">
              <span className="text-[18px] font-extrabold text-[#1d1d1f] leading-none tracking-tight group-hover:text-[#2997ff] transition-colors">AutoImport</span>
            </div>
          </Link>
        </div>
        ...
        <div className="flex-1 flex justify-end gap-5 items-center">
          ...
          <div className="w-[1px] h-4 bg-gray-200 mx-2 hidden sm:block"></div>

          <Link href="/dashboard" className="px-5 py-2 text-[13px] font-bold text-white bg-[#1d1d1f] rounded-full hover:bg-[#2997ff] hover:shadow-[0_4px_15px_rgba(41,151,255,0.4)] transition-all hover:-translate-y-0.5 flex items-center gap-2">
